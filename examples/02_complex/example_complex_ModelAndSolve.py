...
invest_Gaskessel = InvestmentParameters(fixCosts = 1000,  # 1000 € investment costs
...
aKWK  = CHPunit('BHKW2', eta_th = 0.5, eta_el = 0.4, switchOnCosts =  0.01,
...
aKWK2 = BasicLinearTransformer('BHKW2', inputs = [Q_fu], outputs = [P_el, Q_th], segmentsOfFlows = segmentsOfFlows, switchOnCosts = 0.01, on_valuesBeforeBegin = [1])
...
invest_Speicher = InvestmentParameters(fixCosts = 0,  # no fix costs
...
es = EnergySystem(aTimeSeries, dt_last=None) # creating System
...
aCalc = EnergySystemCalculation('Sim1', es, 'pyomo', chosenEsTimeIndexe)
