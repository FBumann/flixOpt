...
aWaermeLast = cSink  ('Wärmelast', sink   = Flow('Q_th_Last', bus = Fernwaerme, nominal_val = 1, val_rel = TS_Q_th_Last))

# TS with explicit defined weight
TS_P_el_Last = cTSraw(P_el_Last, agg_weight = 0.7) # explicit defined weight
aStromLast = cSink('Stromlast', sink = Flow('P_el_Last', bus = Strom, nominal_val = 1, val_rel = TS_P_el_Last))

aKohleTarif = cSource('Kohletarif', source = Flow('Q_Kohle', bus = Kohle, nominal_val = 1000, costsPerFlowHour= {costs: 4.6, CO2: 0.3}))

aGasTarif = cSource('Gastarif', source = Flow('Q_Gas', bus = Gas, nominal_val = 1000, costsPerFlowHour= {costs: gP, CO2: 0.3}))
...
aStromEinspeisung = cSink  ('Einspeisung', sink   = Flow('P_el', bus = Strom, nominal_val = 1000, costsPerFlowHour = p_feed_in))
aStromEinspeisung.sink.costsPerFlowHour[None].setAggWeight(.5)

aStromTarif       = cSource('Stromtarif', source = Flow('P_el', bus = Strom, nominal_val = 1000, costsPerFlowHour= {costs: p_sell, CO2: 0.3}))
...
# Roh-Rechnung:
if doFullCalc:
  calcFull = EnergySystemCalculation('fullModel', es, 'pyomo', chosenEsTimeIndexe)
...
if doSegmentedCalc :
   ...
   calcSegs = EnergySystemCalculation('segModel', es, 'pyomo', chosenEsTimeIndexe)
...
if doAggregatedCalc :    
    calcAgg = EnergySystemCalculation('aggModel', es, 'pyomo')
