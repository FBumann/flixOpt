...
class Variable:
    ...
    def __init__(self, label, len, myMom, baseModel, isBinary=False, value=None, min=None, max=None):
        ...
        myMom.vars_and_eqs.variables.append(
            self)  # TODO: not nice, that this specific thing for energysystems is done here
...
class Vector:
    ...
    def sumOf(self):
        if len == 1:
            print(
                'warning: Vector.sumOf() senceless für Variable ' + self.variable.label + ', because only one vector-element already')
