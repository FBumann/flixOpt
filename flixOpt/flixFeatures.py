...
class cSegment(cFeature) :
    ...
    def declareVarsAndEqs(self, modBox):
        ...
        self.mod.var_onSeg   = TimeSeriesVariable('onSeg_' + str(self.index), aLen, self, modBox, isBinary = True)   # Binär-Variable
        self.mod.var_lambda1 = TimeSeriesVariable('lambda1_' + str(self.index), aLen, self, modBox, min = 0, max = 1) # Wertebereich 0..1
        self.mod.var_lambda2 = TimeSeriesVariable('lambda2_' + str(self.index), aLen, self, modBox, min = 0, max = 1) # Wertebereich 0..1
...
class cFeatureOn(cFeature) :
    ...
    def declareVarsAndEqs(self,modBox):
        ...
        if self.useOn : 
            #Before-Variable:
            self.mod.var_on      = TimeSeriesVariable('on', modBox.nrOfTimeSteps, self.owner, modBox, isBinary = True)
            self.mod.var_on.activateBeforeValues(esBeforeValue = self.on_valuesBeforeBegin[0], beforeValueIsStartValue = False)
            self.mod.var_onHoursSum = Variable('onHoursSum', 1, self.owner, modBox, min = self.onHoursSum_min, max = self.onHoursSum_max) # wenn max/min = None, dann bleibt das frei
        ...
        if self.useOff:
            # off-Var is needed:
            self.mod.var_off = TimeSeriesVariable('off', modBox.nrOfTimeSteps, self.owner, modBox, isBinary = True)
        ...
        if self.useOnHours:
            ...
            self.mod.var_onHours = TimeSeriesVariable('onHours', modBox.nrOfTimeSteps, self.owner, modBox,
                                                      min = 0, max = aMax) # min separat
        # offHours:
        if self.useOffHours:
            aMax = None if (self.offHours_max is None) else self.offHours_max.d_i
            self.mod.var_offHours = TimeSeriesVariable('offHours', modBox.nrOfTimeSteps, self.owner, modBox,
                                                       min = 0, max = aMax) # min separat


        # Var SwitchOn
        if self.useSwitchOn:
            self.mod.var_switchOn  = TimeSeriesVariable('switchOn', modBox.nrOfTimeSteps, self.owner, modBox, isBinary = True)
            self.mod.var_switchOff = TimeSeriesVariable('switchOff', modBox.nrOfTimeSteps, self.owner, modBox, isBinary = True)
...
class cFeature_ShareSum(cFeature): #(ME = ModelingElement)
    ...
    def declareVarsAndEqs(self, modBox):
        ...
        if self.sharesAreTS:
            self.mod.var_sum_TS = TimeSeriesVariable('sum_TS', modBox.nrOfTimeSteps, self, modBox) # TS
