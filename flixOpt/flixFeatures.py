    ...
    def doModeling(self, modBox: EnergySystemModel, timeIndexe):
        ...
        for aVar in self.segmentsOfVars.keys():
            ...
            for aSegment in self.listOfSegments:
                ...
                eqLambda.addSummand(aSegment.vars_and_eqs.var_lambda1,
                                    samplePoint1)  # Spalte 1 (Faktor kann hier Skalar sein oder Vektor)
                eqLambda.addSummand(aSegment.vars_and_eqs.var_lambda2,
...
class cSegment(cFeature):
    ...
    def declareVarsAndEqs(self, modBox):
        ...
        self.vars_and_eqs.var_onSeg = VariableTS('onSeg_' + str(self.index), aLen, self, modBox,
                                                 isBinary=True)  # Binär-Variable
        self.vars_and_eqs.var_lambda1 = VariableTS('lambda1_' + str(self.index), aLen, self, modBox, min=0,
                                                   max=1)  # Wertebereich 0..1
        self.vars_and_eqs.var_lambda2 = VariableTS('lambda2_' + str(self.index), aLen, self, modBox, min=0,
                                                   max=1)  # Wertebereich 0..1
...
class cFeatureOn(cFeature):
    ...
    def declareVarsAndEqs(self, modBox):
        ...
        if self.useOn:
            ...
            self.vars_and_eqs.var_on.activateBeforeValues(esBeforeValue=self.on_valuesBeforeBegin[0],
                                                          beforeValueIsStartValue=False)
            self.vars_and_eqs.var_onHoursSum = Variable('onHoursSum', 1, self.owner, modBox, min=self.onHoursSum_min,
        ...
        # Var SwitchOn
        if self.useSwitchOn:
            self.vars_and_eqs.var_switchOn = VariableTS('switchOn', modBox.nrOfTimeSteps, self.owner, modBox,
                                                        isBinary=True)
    ...
    def __addConstraintsForOn(self, eqsOwner, flowsDefiningOn, modBox, timeIndexe):
        ...
        if nrOfFlows == 1:
            ...
            eq1.addSummand(self.vars_and_eqs.var_on, 1 * np.maximum(modBox.epsilon, min_val),
        ...
        # Bei mehreren Leistungsvariablen:
            ...
            for aFlow in flowsDefiningOn:
                eq1.addSummand(aFlow.vars_and_eqs.var_val, -1, timeIndexe)
            eq1.addSummand(self.vars_and_eqs.var_on, 1 * modBox.epsilon,
                           timeIndexe)  # % aLeistungsVariableMin kann hier Skalar oder Zeitreihe sein!
    ...
    def __addConstraintsForSwitchOnSwitchOff(self, eqsOwner, modBox, timeIndexe):
        ...
        eq_SwitchOnOffAtFirstTime.addRightSide(
            -self.vars_and_eqs.var_on.beforeVal())  # letztes Element der Before-Werte nutzen,  Anmerkung: wäre besser auf lhs aufgehoben
    ...
    def addShareToGlobals(self, globalComp, modBox):
        ...
        if self.switchOnCosts is not None:  # and any(self.switchOnCosts.d_i != 0):
            globalComp.addShareToOperation('switchOnCosts', shareHolder, self.vars_and_eqs.var_switchOn,
                                           self.switchOnCosts, 1)
        # Betriebskosten:
        if self.costsPerRunningHour is not None:  # and any(self.costsPerRunningHour):
            globalComp.addShareToOperation('costsPerRunningHour', shareHolder, self.vars_and_eqs.var_on,
...
class cFeatureInvest(cFeature):
    ...
    def __init__(self, nameOfInvestmentSize, owner, investArgs: InvestmentParameters, min_rel, max_rel, val_rel,
                 investmentSize, featureOn=None):
        '''
    ...
    def _add_fixEq_of_definingVar_with_var_investmentSize(self, modBox):
        ...
        self.eq_fix_via_investmentSize.addSummand(self.vars_and_eqs.var_investmentSize,
                                                  np.multiply(-1, self.val_rel.d_i))
    ...
    def _add_defining_var_isInvested(self, modBox):
        ...
        # wenn nicht fix, dann Bereich:
            ...
            self.eq_isInvested_1.addSummand(self.vars_and_eqs.var_investmentSize, 1)
            self.eq_isInvested_1.addSummand(self.vars_and_eqs.var_isInvested,
            ...
            self.eq_isInvested_2.addSummand(self.vars_and_eqs.var_isInvested,
                                            max(modBox.epsilon, self.args.min_investmentSize))
    ...
    def addShareToGlobals(self, globalComp, modBox):
        ...
        if not (self.args.fixCosts is None) and self.args.fixCosts != 0:
            if self.args.investment_is_optional:
                ...
                globalComp.addShareToInvest('fixCosts', self.owner, self.vars_and_eqs.var_isInvested,
                                            self.args.fixCosts, 1)
            else:
                # share: + fixCosts
        ...
        if not (self.args.divestCosts is None) and self.args.divestCost != 0:
            if self.args.investment_is_optional:
                ...
                globalComp.addShareToInvest('divestCosts_cancellation', self.owner, self.vars_and_eqs.var_isInvested,
        ...
        if not (self.args.specificCosts is None):
            ...
            globalComp.addShareToInvest('specificCosts', self.owner, self.vars_and_eqs.var_investmentSize,
